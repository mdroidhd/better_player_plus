...
- (void)cleanupPictureInPictureResourcesEmitEvent:(BOOL)emitEvent {
...
- (void)usePlayerLayer: (CGRect) frame
{
    if( _player )
    {
        UIViewController *vc = [self activeRootViewController];
        if (!vc) {
            if (_eventSink != nil) {
                _eventSink([FlutterError
                            errorWithCode:@"PiPError"
                            message:@"Failed to find active root view controller for PiP."
                            details:nil]);
            }
            return;
        }
...
- (void)disablePictureInPicture
{
    ...
    if (@available(iOS 9.0, *)) {
        if (!_pipController || ![_pipController isPictureInPictureActive]) {
            [self cleanupPictureInPictureResourcesEmitEvent:YES];
        }
    } else {
        [self cleanupPictureInPictureResourcesEmitEvent:YES];
    }
...
- (void)pictureInPictureController:(AVPictureInPictureController *)pictureInPictureController failedToStartPictureInPictureWithError:(NSError *)error {
    [self cleanupPictureInPictureResourcesEmitEvent:NO];
    if (_eventSink != nil) {
        _eventSink([FlutterError
                    errorWithCode:@"PiPError"
                    message:[@"Failed to start PiP: " stringByAppendingString:error.localizedDescription ?: @"Unknown error"]
                    details:nil]);
    }
}
