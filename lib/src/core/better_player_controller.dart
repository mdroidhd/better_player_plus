...
class BetterPlayerController {
  ...
  Future _setupDataSource(BetterPlayerDataSource betterPlayerDataSource) async {
    switch (betterPlayerDataSource.type) {
      ...
      case BetterPlayerDataSourceType.memory:
        ...
        break;
      case BetterPlayerDataSourceType.merged:
        final mergedSources = _betterPlayerDataSource!.mergedSources!;
        final List<Map<String, dynamic>> sourcesList = mergedSources.map((
          source,
        ) {
          return _buildSubSourceMap(source);
        }).toList();
  ...
  ///Build a map representing a single sub-source for MergingMediaSource.
  ///Serializes a [BetterPlayerDataSource] into a map that can be passed
  ///through the MethodChannel to Android native code.
  Map<String, dynamic> _buildSubSourceMap(BetterPlayerDataSource source) {
    switch (source.type) {
      case BetterPlayerDataSourceType.network:
        return <String, dynamic>{
          ...
          'cacheKey': source.cacheConfiguration?.key,
          'licenseUrl': source.drmConfiguration?.licenseUrl,
          'drmHeaders': source.drmConfiguration?.headers,
          'clearKey': source.drmConfiguration?.clearKey,
      ...
      default:
        throw ArgumentError(
          'Unsupported sub-source type for merged data source: ${source.type}',
        );
    }
  }
